...
func upsertUser(ctx context.Context, sqlTx pgx.Tx, userId int, metadata string, blockNumber int64) error {
	...
	_, err := sqlTx.Exec(ctx, `
		INSERT INTO users (
			...
			@metadata::jsonb->'data'->>'profile_type',
			...
			@metadata::jsonb->'data'->>'spl_usdc_payout_wallet',
			@blockNumber,
            NOW(),
            NOW()
			...
			profile_type = CASE WHEN @metadata::jsonb->'data' ? 'profile_type' THEN EXCLUDED.profile_type ELSE users.profile_type END,
			...
			payout_wallet = CASE WHEN @metadata::jsonb->'data' ? 'spl_usdc_payout_wallet' THEN EXCLUDED.payout_wallet ELSE users.payout_wallet END,
