...
func upsertTrack(ctx context.Context, sqlTx pgx.Tx, id int, metadata string, blockNumber int64) error {
	...
	_, err := sqlTx.Exec(ctx, `
		...
		ON CONFLICT (id) DO UPDATE SET
			...
			cid = CASE WHEN @metadata::jsonb ? 'cid' THEN EXCLUDED.cid ELSE tracks.cid END,
